...
namespace _125CNX03_Nhom6_CK.DAL.Repositories
{
    public class LienHeRepository : ILienHeRepository
    {
        private readonly string _filePath =
                    Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "Data", "LienHe.xml");
        private readonly string _tableName = "LienHe";
        ...
        public List<XElement> GetUnreadMessages()
        {
            return GetAll().Where(m =>
                m.Element("DaXem") != null &&
                bool.TryParse(m.Element("DaXem").Value, out var daXem) &&
                !daXem).ToList();
        }
        ...
        public void MarkAsRead(int id)
        {
            ...
            if (message != null)
            {
                message.Element("DaXem").Value = "true";
                Update(message);
            }
        }
    }
