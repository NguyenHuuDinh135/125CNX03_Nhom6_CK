...
namespace _125CNX03_Nhom6_CK.DAL.Repositories
{
    public class PhuongThucThanhToanRepository : IPhuongThucThanhToanRepository
    {
        private readonly string _filePath =
            Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "Data", "PhuongThucThanhToan.xml"); 
        private readonly string _tableName = "PhuongThucThanhToan";
        ...
        public List<XElement> GetByUserId(int userId)
        {
            return GetAll().Where(p =>
                p.Element("MaNguoiDung") != null &&
                int.TryParse(p.Element("MaNguoiDung").Value, out var userIdValue) &&
                userIdValue == userId).ToList();
        }

        public XElement GetByCardNumber(string cardNumber)
        {
            return GetAll().FirstOrDefault(p =>
                p.Element("MaThe")?.Value == cardNumber);
        }
    }
}